...
export const Step2Output = () => {
  ...
  const increaseInCowProductivity = useAppSelector(
    (state) => state.increaseInCowProductivity.increaseInCowProductivity
  )
  ...
  return (
    ...
    >
        ...
        <Table>
          ...
          <TableBody>
            {Object.entries(feedNamesInRussian).map(([key, feedName]) => {
              ...
              return (
                <TableRow key={key}>
                  ...
                  <TableCell>
                    {formatNumber(feed.additionalYoungCattle)}
                  </TableCell>
                  <TableCell>
                    {calculatePercentage(feed.additionalCows, totalCows)}
                  </TableCell>
                  <TableCell>
                    {calculatePercentage(
                      feed.additionalYoungCattle,
                      totalYoungCattle
                    )}
                  </TableCell>
                </TableRow>
            ...
            <TableRow>
              ...
              <TableCell>{formatNumber(totalCows)}</TableCell>{' '}
              {/* Сумма для "ц.к.ед (Коров)" */}
              <TableCell>-</TableCell> {/* Столбцы для "ц (КРС)" */}
              <TableCell>{formatNumber(totalYoungCattle)}</TableCell>{' '}
              {/* Сумма для "ц.к.ед (КРС)" */}
              <TableCell>100%</TableCell>{' '}
              {/* Процентное соотношение для "ц.к.ед (Коров)" */}
              <TableCell>100%</TableCell>{' '}
              {/* Процентное соотношение для "ц.к.ед (КРС)" */}
            </TableRow>
          </TableBody>
        </Table>
      </TableContainer>
      ...
      <Typography
        ...
        gutterBottom
      >
        Прирост продуктивности коров от базового уровня –{' '}
        {increaseInCowProductivity} ц
      </Typography>
    </Box>
  )
}
