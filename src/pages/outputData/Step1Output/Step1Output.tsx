...
const headers = [
  'Культуры',
  'Расчетное значение площадь',
  'Процентное соотношение площадь',
]
...
export const Step1Output = (props: Props) => {
  ...
  const calculatePercentage = (cultureKey: string) => {
    const cultureSquare =
      culturesState[cultureKey as keyof typeof cultures]?.square || 0
    const totalSquare = landResources.arableLand

    if (
      cultureKey === 'hayImprovedHayfieldsAndPastures' ||
      cultureKey === 'haylageImprovedHayfieldsAndPastures'
    ) {
      const improvedTotal = landResources.hayfieldsAndPastureImproved
      return (cultureSquare / improvedTotal) * 100
    } else if (
      cultureKey === 'haylageNaturalHayfieldsAndPastures' ||
      cultureKey === 'greenFodderNaturalHayfieldsAndPastures'
    ) {
        ...
        <TableBody>
          {Object.entries(cultures).map(([key, label]) => (
            <TableRow key={key}>
              ...
              <TableCell>
                {Math.round(
                  culturesState[key as keyof typeof cultures].square || 0
                )}
              </TableCell>
